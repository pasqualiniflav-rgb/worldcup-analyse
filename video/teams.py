...
def _torso_color(frame, x, y, w, h):
    """Couleur moyenne de la zone torse (bande centrale haute du bbox)."""
    H, W = frame.shape[:2]
    x1 = int(max(0, x - w * 0.25)); x2 = int(min(W, x + w * 0.25))
    y1 = int(max(0, y - h * 0.30)); y2 = int(min(H, y - h * 0.05))
    if x2 <= x1 or y2 <= y1:
        return None
...
def assign_teams(video_path: Path, tracking_csv: Path, cfg: dict | None = None) -> Path:
    ...
    labels = KMeans(n_clusters=2, n_init=10, random_state=0).fit_predict(feats)
